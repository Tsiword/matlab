% clear all;
% close all;
load('E:\qnj\科研\03复合能量收集\建模与仿真\仿真程序\我的模型\EnergySystem\Datas\voc_SC.mat');
t1 = rand(1,2);
t2 = rand(1,8);
...
t_c = lsqcurvefit(@SC_charge,t1,s_c,v_chargen);
t_d = lsqcurvefit(@sv,t2,s_d,v_dischargen);
figure(1),plot(s_c,v_chargen,':',s_c,sv(t_c,s_c));
legend('measured','modeled','Location','SouthEast');
xlabel('SOC'),ylabel('Voc(V)'),title('锂电池恒流充电曲线');
figure(2),plot(s_d,v_dischargen,':',s_d,sv(t_d,s_d));
set(gca,'XDir','reverse');  
legend('measured','modeled');
xlabel('SOC'),ylabel('Voc(V)'),title('锂电池恒流放电曲线');